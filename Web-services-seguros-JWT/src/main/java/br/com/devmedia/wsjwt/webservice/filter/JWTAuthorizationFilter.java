...
@Provider
@Priority(Priorities.AUTHORIZATION) //Segunda prioridade, depois da autenticação
public class JWTAuthorizationFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        try {
            ...
            String usuario = requestContext.getSecurityContext()
                    .getUserPrincipal().getName();
            throw new UnauthorizedException("Usuário " + usuario + " não tem autorização para executar essa funcionalidade.");
        }
    }
